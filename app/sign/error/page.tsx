...
type Props = {
  searchParams: Promise<{ error: string; email?: string; emailcheck?: string; emailType?: 'regist' | 'reset-password' }>;
};
      ...
      <div className='text-center'>
        ...
        <div className='item-center flex flex-col gap-3'>
          ...
          {error === 'CheckEmail' && email && emailcheck && (
            <>
              <Divider label='or' />
              <ResendRegist email={email} emailcheck={emailcheck} emailType={emailType} />
            </>
          )}
        </div>
      </div>
